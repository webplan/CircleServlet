...
public class Servlet {
    ...
    public static Connection connectSQL() {
        if (con == null) {
            String url = "jdbc:mysql://localhost:3306/Circle?useUnicode=true&characterEncoding=UTF-8&allowMultiQueries=true";
...
//            boolean flag = stmt.execute(String sql) ;
    ...
    public static JSONObject searchUser(String account, String token, String nickname) {
        Connection con = connectSQL();
        JSONObject obj = new JSONObject();
        String sql = "SELECT * FROM User WHERE nickname LIKE '%" + nickname + "%'";
        try {
            java.sql.Statement stmt = con.createStatement();
            boolean istoken = Check.CheckToken(account, con, token);
            if (!istoken) {
    ...
    public static JSONObject updateInfo(String account, String token, String nickname, String avatar, int gender,
        ...
        try {
            ...
            if (old_pwd_md5 != null && new_pwd_md5 != null) {
                if (!Check.checkPassword(account, old_pwd_md5, con)) {
                    obj.put("status", 0);
                    return obj;
                }

                sql = "UPDATE User SET password='" + new_pwd_md5 +
                ...
                sql = "UPDATE User SET avatarUrl='" + avatarUrl +
                        "' WHERE account = '" + account + "'";
                int rows = stmt.executeUpdate(sql);
                if (rows != 1)
                    obj.put("status", 0);
...
//            boolean flag = stmt.execute(String sql) ;
    ...
    public static JSONObject userInfo(String account, String token, String user_account) {
        ...
        try {
            java.sql.Statement stmt = con.createStatement();
            boolean istoken = Check.CheckToken(account, con, token);
            boolean isFriend = Check.checkFriend(account, user_account, con);
            if (!istoken) {
                obj.put("status", 2);
                return obj;
            }
